	...
	<li data-value="<? echo $arOnePhoto['ID']; ?>" style="width: <? echo $strOneWidth; ?>; padding-top: <? echo $strOneWidth; ?>;">
		<span class="cnt">
				<span class="cnt_item">
					...
					<img src="<?=$arOnePhoto['SRC']?>" style="width: 84px; height: 84px;" alt="<?=$arResult["NAME"]?>">
				</span>
		</span>
	</li>
<?
...
?>
	...
	<span class="bx_bigimages_aligner"><img class="my-photo" id="<? echo $arItemIDs['PICT']; ?>" src="<? echo $arFirstPhoto['SRC']; ?>" alt="<? echo $strAlt; ?>" title="<? echo $strTitle; ?>"></span>
<?
...
?>
				<li data-treevalue="<? echo $arProp['ID'].'_'.$arOneValue['ID']; ?>" data-onevalue="<? echo $arOneValue['ID']; ?>" class="sku_prop_li">
...
<?//=
		...
		$sectionID = 0;
		$articleText = "";
		$res = CIBlockElement::GetList(array(), array("IBLOCK_ID" => 12, "PROPERTY_LINK_PRODUCT" => $arResult["ID"]), false, false, array("NAME", "PREVIEW_PICTURE","DETAIL_TEXT", "PROPERTY_LINK_PRODUCT", "PROPERTY_LINK_SECTION"));
		if ($row = $res -> fetch()) {
			//pp($row);
			$articleText = $row["DETAIL_TEXT"];
			$articleImage = CFile::GetFileArray($row["PREVIEW_PICTURE"]);
		}
		else {
			/*$sections = array();
			$res = CIBlockElement::GetElementGroups($arResult["ID"], array("ID"));
			while ($row = $res -> fetch()) {
				$sections[] = $row["ID"];
			}
			pp($sections);*/
			//$sections = $arResult["IBLOCK_SECTION_ID"];

			$sections = array();
			foreach ($arResult["SECTION"]["PATH"] as $item) {
				$sections[] = $item["ID"];
			}
			$res = CIBlockElement::GetList(array(), array("IBLOCK_ID" => 12, "PROPERTY_LINK_SECTION" => $sections), false, false, array("NAME", "PREVIEW_PICTURE", "DETAIL_TEXT", "PROPERTY_LINK_PRODUCT", "PROPERTY_LINK_SECTION"));
			if ($row = $res -> fetch()) {
			$articleText = $row["DETAIL_TEXT"];
			$articleImage = CFile::GetFileArray($row["PREVIEW_PICTURE"]);
				//pp($row);
...
//print_r($articleImage);
		?>


		<script>
			$(document).ready(function() {
				$(".fancybox-articles").fancybox();
			});
		</script>
		<script>
			$(document).ready(function() {
				$("#article-container").click(
					function(){
		...
		<?if ($articleText || $articleImage) {?>
		<a href="#article-container" class="selection-rule fancybox-articles"> <?//=/include/catalog/element/rules.php fancybox.ajax?>
			<span>Правила подбора</span>
		</a>
		<?}?>
		<div id="article-container"><?=$articleText?><img src="<?=$articleImage['SRC']?>" alt="<?=$articleText?>" ></div>
	...
	<div class="item_buttons vam">
		<span class="item_buttons_counter_block">
			...
			<input id="<? echo $arItemIDs['QUANTITY']; ?>" type="text" class="tac transparent_input" value="<? echo (isset($arResult['OFFERS']) && !empty($arResult['OFFERS'])
				...
				); ?>"><span style="padding-right:15px;" class="bx_cnt_desc" id="<? echo $arItemIDs['QUANTITY_MEASURE']; ?>"><? echo (isset($arResult['CATALOG_MEASURE_NAME']) ? $arResult['CATALOG_MEASURE_NAME'] : ''); ?></span>
...
?>
			...
			<div class="clb">
				...
				<p class="card-p"><span>Самовывоз доступен:</span> 
				<?php
					$dates = date('j.m.Y');
				...
				?>;</p>
				<p class="card-p"><span>Доставка:</span> 
				<?php
					$dates = date('j.m.Y');
					$date = date_create("$dates");
					...
					echo date_format($date, 'j.m.Y');
				?>;</p>
				<p class="card-p"><span>Стоимость доставки:</span> от 260 руб.</p>
			</div>
		</div>
...
?>
		...
		<div class="bx_rb" style="display:none;">
<div class="item_info_section">
<?
...
?>
		...
		<div class="bx_lb" style="width:93%;">
			<ul class="tabs_card">
		<li><a href="#tab_1"><span>Описание</span></a></li>
		<!--<li><a href="#tab_2"><span>Характеристики</span></a></li>-->
		<li><a href="#tab_3"><span>Доставка</span></a></li>
		<li><a href="#tab_4"><span>Оплата</span></a></li>
		<li><a href="#tab_5"><span>Отзывы (<?if($arResult["PROPERTIES"]["BLOG_COMMENTS_CNT"]["VALUE"]) echo $arResult["PROPERTIES"]["BLOG_COMMENTS_CNT"]["VALUE"]; else echo 0;?>)</span></a></li>
	</ul>
	<div class="tab_container_card">
		<div id="tab_1" class="tab_content_card">
			<?=$arResult["DETAIL_TEXT"]?>
		</div>
		<div id="tab_2" class="tab_content_card chars"><?
			foreach($arResult["DISPLAY_PROPERTIES"] as $arProperty) {
				?><p><span class="name"><?=$arProperty["NAME"]?></span><span class="value"><?=$arProperty["VALUE"]?></span></p><?
			}
		?></div>
		<div id="tab_3" class="tab_content_card">
			<? include ("../include/element/dostavka.php");?>
		</div>
		<div id="tab_4" class="tab_content_card">
			<? include ("../include/element/oplata.php");?>
		</div>

		<div id="tab_5" class="tab_content_card">
			<?$APPLICATION->IncludeComponent(
...
<?
	...
	$arJSParams = array(
		'CONFIG' => array(
			...
			'SHOW_DISCOUNT_PERCENT' => ($arParams['SHOW_DISCOUNT_PERCENT'] == 'Y'),
			'SHOW_OLD_PRICE' => ($arParams['SHOW_OLD_PRICE'] == 'Y'),
			'DISPLAY_COMPARE' => ($arParams['DISPLAY_COMPARE'] == 'N'),
			'MAIN_PICTURE_MODE' => $arParams['DETAIL_PICTURE_MODE'],
			'SHOW_BASIS_PRICE' => ($arParams['SHOW_BASIS_PRICE'] == 'Y'),
			'ADD_TO_BASKET_ACTION' => $arParams['ADD_TO_BASKET_ACTION'],
			'SHOW_CLOSE_POPUP' => ($arParams['SHOW_CLOSE_POPUP'] == 'Y')
...
<?
//echo "<pre>";
//print_r($arResult["PROPERTIES"]["OTHER_ORDER"]);
//echo "</pre>";
?><?//=$arResult["PROPERTIES"]["OTHER_ORDER"]["VALUE"];?>
<!-- С этим товаром покупают. Карусель-->
		<?
		//Выборка из разделов
		//Получаем ID нужных разделов из свойства
		
		if(empty($arResult["PROPERTIES"]["OTHER_ORDER"]["VALUE"]))
		{
		$sect = $arResult['IBLOCK_SECTION_ID'];
		$arFilter = Array('IBLOCK_ID'=>$arParams['IBLOCK_ID'],'ID'=>$sect, 'GLOBAL_ACTIVE'=>'Y');
		$db_list = CIBlockSection::GetList(Array("timestamp_x"=>"DESC"), $arFilter, false, Array("UF_OTHER_ORDER"));
		$uf_value = $db_list->fetch();
		$arSectionID = $uf_value['UF_OTHER_ORDER'];
			if(!empty($arSectionID)){
				//Собираем массив с разделами
				echo "<div class=\"title_other_order\"><h2><span>С !этим товаром покупают</span></h2></div><ul id=\"flexiselDemo2\">";
				$arFilter2 = Array('IBLOCK_ID'=>$arParams['IBLOCK_ID'],'ID'=>$arSectionID);
				$lxSection = CIBlockSection::GetList(array(), $arFilter2);
				while($lxRes = $lxSection->Fetch())
				{
					$rsFile = CFile::GetPath($lxRes['PICTURE']);
					echo "<li><p>".$lxRes['NAME']."</p><a href=\"/store/".$lxRes['CODE']."/\"><img src=\"".$rsFile."\"/></a></li>";
				}
				echo "</ul>";
			}
		}
		...
		{
		//Выборка из карточки товара
		$arSectionID = $arResult["PROPERTIES"]["OTHER_ORDER"]["VALUE"];
		echo "<div class=\"title_other_order\"><h2><span>С этим товаром покупают</span></h2></div><ul id=\"flexiselDemo2\">";
		$arFilter2 = Array('IBLOCK_ID'=>$arParams['IBLOCK_ID'],'ID'=>$arSectionID);
		$lxSection = CIBlockSection::GetList(array(), $arFilter2);
		while($lxRes = $lxSection->Fetch())
...
<!-- С этим товаров покупают. Карусель КОНЕЦ-->
<script type="text/javascript">
