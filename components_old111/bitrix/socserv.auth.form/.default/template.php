...
<div class="social-lk">
	...
	?>
	<a title="<?=htmlspecialcharsbx($service["NAME"])?>" href="javascript:void(0)" <?=$onClickEvent?> id="bx_auth_href_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>" class="<?=htmlspecialcharsbx($service["ICON"])?>"></a>
	<?endforeach?>
</div>
<?endif;?>
<div class="bx-auth">
	<form method="post" name="bx_auth_services<?=$arParams["SUFFIX"]?>" target="_top" action="<?=$arParams["AUTH_URL"]?>">
		<?if($arParams["~FOR_SPLIT"] != 'Y'):?>
			<div class="social-lk">
				<?foreach($arAuthServices as $service):?>
					<a href="javascript:void(0)" onclick="BxShowAuthService('<?=$service["ID"]?>', '<?=$arParams["SUFFIX"]?>')" id="bx_auth_href_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>" class="<?=htmlspecialcharsbx($service["ICON"])?>"></a>
				<?endforeach?>
			</div>
		<?endif;?>
