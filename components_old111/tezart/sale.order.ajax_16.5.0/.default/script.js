...
function loadDelivery() {
    var url = window.location.pathname + "?deliveryLoad=ok";
    //url = url.match("?") ? url + "&deliveryLoad=ok" : url + "?deliveryLoad=ok";

    $.ajax({
        type: "GET",
        url: url, // /local/templates/tpl/ajax/coupon.php
        data: "",
        dataType: "html",
        success: function (data) {                           //console.log(block_id); console.log($(block_id));
            //$("#coupons_block").html($(data).find("#coupons_block").first().html());
            //$("#delivery-container").html($(data).find("#delivery-container").html());
            $(".cssload-container").remove();
            $("div.order-page-left").append($(data).find("p.info-delivery ~ *"));  //p.info-delivery
            //console.log($(data).find("p.info-delivery ~ *").html());
