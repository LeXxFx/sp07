<!-- this!!! --><?if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();
/** @var array $arParams */
/** @var array $arResult */
...
<div class="row-product">
	<p class="title">
		<span style="font-size:26px; color:#7d7878;"><?if($arParams["SECTION_TITLE"] != "") echo $arParams["SECTION_TITLE"]; else echo $arResult["NAME"]?>
		</span>
		</p>
	<div class="sub-category">
		<?if($arParams["SECTION_TITLE_LINK"] != ""):?><a href="<?=$arParams["SECTION_TITLE_LINK"]?>">Посмотреть все</a><?endif;?>
	</div>
...
if (!empty($arResult['ITEMS']))
{
	...
	{ ?><div class="lx-products">
<div class="bx-section-desc <? echo $templateData['TEMPLATE_CLASS']; ?>">
	<p class="bx-section-desc-post"><!-- la-la-la -->
...
<!-- ? -->	
	...
	unset($minPrice);
	?></div></div><?
	/*if($arItem['CATALOG_SUBSCRIBE'] == 'N')
		$showSubscribeBtn = true;
	else
		$showSubscribeBtn = false;*/
	$compareBtnMessage = ($arParams['MESS_BTN_COMPARE'] != '' ? $arParams['MESS_BTN_COMPARE'] : GetMessage('CT_BCS_TPL_MESS_BTN_COMPARE'));
	if (empty($arItem['OFFERS']) || !isset($arItem['OFFERS']))
	{
		?><div class="catalog_quantity_buy"><?
		if ($arItem['CAN_BUY'])
		{
			if ('Y' == $arParams['USE_PRODUCT_QUANTITY'])
			{
			?>
		<div class="catalog_quantity_block"><!-- canbuy: <? echo $arItem['CAN_BUY']; ?>  -->
			<div class="catalog_quantity_button_container"><a id="<? echo $arItemIDs['QUANTITY_DOWN']; ?>" href="javascript:void(0)" class="catalog_quantity_down" rel="nofollow">-</a></div>
			<input type="text" class="catalog_quantity_num" id="<? echo $arItemIDs['QUANTITY']; ?>" name="<? echo $arParams["PRODUCT_QUANTITY_VARIABLE"]; ?>" value="<? echo $arItem['CATALOG_MEASURE_RATIO']; ?>">
		<span id="<? echo $arItemIDs['QUANTITY_MEASURE']; ?>"></span>
		<div class="catalog_quantity_button_container"><a id="<? echo $arItemIDs['QUANTITY_UP']; ?>" href="javascript:void(0)" class="catalog_quantity_up" rel="nofollow">+</a></div>
			
		</div>
			<?
			}
			?>
		<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="catalog_buy_block"><!--this is first -->
			<a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="catalog_buy_button catalog_buy_button buy-product addtobasket" href="javascript:void(0)" rel="nofollow"><?
...
var <? echo $strObName; ?> = new JCCatalogSection(<? echo CUtil::PhpToJSObject($arJSParams, false, true); ?>);
	...
	{ 
		...
		{ 
			...
			?>
		<div class="catalog_quantity_buy"><!--  <? echo $arItem['OFFERS_SELECTED'];?>  -->
			<?
			if ('Y' == $arParams['USE_PRODUCT_QUANTITY'])
			{
			?>
		<div class="catalog_quantity_block">
			<div class="catalog_quantity_button_container"><a id="<? echo $arItemIDs['QUANTITY_DOWN']; ?>" href="javascript:void(0)" class="catalog_quantity_down" rel="nofollow">-</a></div>
			<input type="text" class="catalog_quantity_num" id="<? echo $arItemIDs['QUANTITY']; ?>" name="<? echo $arParams["PRODUCT_QUANTITY_VARIABLE"]; ?>" value="<? echo $arItem['CATALOG_MEASURE_RATIO']; ?>">
		<span id="<? echo $arItemIDs['QUANTITY_MEASURE']; ?>"></span>
		<div class="catalog_quantity_button_container"><a id="<? echo $arItemIDs['QUANTITY_UP']; ?>" href="javascript:void(0)" class="catalog_quantity_up" rel="nofollow">+</a></div>
		...
		<div id="<? echo $arItemIDs['NOT_AVAILABLE_MESS']; ?>" class="bx_catalog_item_controls_blockone" style="display: <? echo ($canBuy ? 'none' : ''); ?>;"><span class="bx_notavailable"><?
			echo ('' != $arParams['MESS_NOT_AVAILABLE'] ? $arParams['MESS_NOT_AVAILABLE'] : GetMessage('CT_BCS_TPL_MESS_PRODUCT_NOT_AVAILABLE'));
		?></span></div>
		<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="catalog_buy_block" style="display: <? echo ($canBuy ? '' : 'none'); ?>;">
			<a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="catalog_buy_button buy-product addtobasket js_buy_btn" href="javascript:void(0)" rel="nofollow"><?
			if ($arParams['ADD_TO_BASKET_ACTION'] == 'BUY')
			{
				echo ('' != $arParams['MESS_BTN_BUY'] ? $arParams['MESS_BTN_BUY'] : GetMessage('CT_BCS_TPL_MESS_BTN_BUY'));
			}
		...
		if ('Y' == $arParams['PRODUCT_DISPLAY_MODE'])
		{
			if (!empty($arItem['OFFERS_PROP']))
			{
				...
				foreach ($skuTemplate as $propId => $propTemplate)
				{
					...
					unset($valueCount);
					echo '<div>', str_replace(array('#ITEM#_prop_', '#WIDTH#'), array($arItemIDs['PROP'], 'auto'), $rowTemplate['START']);
					foreach ($propTemplate['ITEMS'] as $value => $valueItem)
					{
						if (!isset($arItem['SKU_TREE_VALUES'][$propId][$value]))
							continue;
						echo str_replace(array('#ITEM#_prop_', '#WIDTH#'), array($arItemIDs['PROP'], 'auto'), $valueItem);
					}
