...
foreach ($arResult['ITEMS'] as $key => $arItem){
	$arCatalog = CCatalog::GetByIDExt(intval(9));
	$arFilter = array('IBLOCK_ID' => $arCatalog['OFFERS_IBLOCK_ID'], 'PROPERTY_'.$arCatalog['OFFERS_PROPERTY_ID'] => $arItem['ID'], 'ACTIVE' => 'Y');
	$rsOffers = CIBlockElement::GetList(array(),$arFilter);

	$arResult['ITEMS'][$key]['OFFERS2']=$rsOffers->Fetch();
}
...
{
   $arResult['SECTION_ID_MAIN'] = $arParentSection['ID'];
   $arResult['SECTION_NAME_MAIN'] = $arParentSection['NAME'];
}
//