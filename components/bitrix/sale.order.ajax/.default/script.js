$(function() {
	// onclick="submitForm2('Y'); return false;"

	$('.Checkout_Order').click(function(){
		// console.log(CheckOrderProps());
		if (CheckOrderProps())
			submitForm2('Y');
		return false;
	});

	var CheckOrderProps = function(){
		var check = true;
		var toFocus=false;
		$('form[name=ORDER_FORM] input.not_empty').each(function(index,el){
			if ($(el).context.value.length==0){
				if (!toFocus)
					toFocus=$(el);

				check=false;
				$(el).parent().find('.err_empty').show().fadeOut(3000);
				// return false;
			}
		});
		if (toFocus)
